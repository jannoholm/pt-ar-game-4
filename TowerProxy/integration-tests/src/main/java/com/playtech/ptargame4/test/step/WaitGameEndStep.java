...
import com.playtech.ptargame4.api.lobby.GameStatus;
...
import com.playtech.ptargame.common.task.state.OneStepState;
import com.playtech.ptargame4.test.ContextConstants;
import com.playtech.ptargame4.test.step.common.AbstractStep;
import com.playtech.ptargame4.test.step.substeps.RegisterForGameLobbyUpdateStep;
...
public class WaitGameEndStep extends AbstractStep {
    public WaitGameEndStep(LogicResources logicResources) {
    ...
    public boolean canExecute(Task task) {
        if (task.getCurrentState() == OneStepState.FINAL) {
            ...
            if (update != null && update.getGameStatus() == GameStatus.FINISHED) {
                return true;
            }
        }
        return false;
